...
export default function RoutesPage() {
  ...
  useEffect(() => {
    // Don't load if from/to are not provided yet
    if (!from || !to) {
      setLoading(false);
      return;
    }
  ...
  const handleSearch = async (startLocation, endLocation) => {
    ...
    if (startLocation) params.set("from", startLocation);
    if (endLocation) params.set("to", endLocation);
    ...
    <div className="app-container">
      <header className="sticky-header">
        ...
        <div className="row-between">
          ...
          <h1 style={{ fontSize: "1.2rem", fontWeight: 800, margin: 0 }}>Routes</h1>
      ...
      <main className="scrollable-content">
        {/* Search Section */}
        <div style={{ padding: "20px", paddingBottom: plan ? "0" : "20px" }}>
          <div className="premium-card" style={{ margin: "0 0 24px 0", padding: "20px", display: "grid", gap: "12px" }}>
            <SearchForm 
              from={from} 
              to={to} 
              loading={loading}
        ...
        {!from && !to && !plan && (
          <div style={{ padding: "20px", textAlign: "center", color: "var(--text-muted)", marginTop: "40px" }}>
            ...
            <p style={{ fontSize: "1rem", margin: "0" }}>Search for routes to get started</p>
            <p style={{ fontSize: "0.9rem", margin: "8px 0 0" }}>Enter your location and destination above</p>
          </div>
        )}
      </main>
...
function SearchForm({ from, to, loading, onSearch }) {
  const [startInput, setStartInput] = useState(from || "");
  const [endInput, setEndInput] = useState(to || "");
    ...
    <>
      <div style={{ position: "relative" }}>
        ...
        <input 
          ...
          style={{ width: "100%", padding: "14px 14px 14px 40px", borderRadius: "14px", border: "1px solid var(--border-medium)", background: "var(--bg-app)", fontSize: "0.95rem", outline: "none" }}
        />
      </div>
      <div style={{ position: "relative" }}>
        <span style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", color: "var(--accent)" }}>
      ...
      >
        {loading ? "Searching..." : "Find Best Routes"}
      </button>
    </>
  );
}
